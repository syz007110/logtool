<template>
  <div class="dict-container">
    ...
    <el-dialog 
      ...
      :title="isEdit ? $t('configManagement.common.editItem') : $t('configManagement.statuses.add')" 
      width="640px"
      ...
      <template #header>
        <div class="dialog-header">
          <div class="dialog-title-wrapper">
            <h3 class="dialog-title">{{ isEdit ? $t('configManagement.common.editItem') : $t('configManagement.statuses.add') }}</h3>
...
export default {
  ...
  setup() {
    ...
    const formRules = {
      ...
      name: [
        { required: true, message: isZhCN.value ? t('configManagement.statuses.validation.nameZhRequired') : t('configManagement.statuses.validation.nameEnRequired'), trigger: 'blur' }
      ]
    }
    ...
    const handleEdit = async (row) => {
      ...
      // 加载映射值
      try {
        const resp = await api.faultCaseStatuses.getMappings(row.id)
    ...
    const handleDelete = async (row) => {
      try {
        const confirmed = await confirmDelete(row, {
          message: t('configManagement.statuses.deleteConfirm'),
    ...
    const handleSubmit = async () => {
      try {
        ...
        let resp
        if (isEdit.value) {
          resp = await api.faultCaseStatuses.update(formData.id, payload)
        } else {
          resp = await api.faultCaseStatuses.create(payload)
        }
        
        if (resp.data?.success) {
          const itemId = isEdit.value ? formData.id : resp.data.status?.id
          
          // 保存映射值
          if (itemId && formData.mappingValues.length > 0) {
            try {
              // 先获取现有映射
              const mappingsResp = await api.faultCaseStatuses.getMappings(itemId)
              ...
              for (const mapping of existingMappings) {
                if (!formData.mappingValues.includes(mapping.source_value)) {
                  await api.faultCaseStatuses.deleteMapping(mapping.id)
                }
              }
              ...
              for (const value of formData.mappingValues) {
                if (!existingMappings.find(m => m.source_value === value)) {
                  await api.faultCaseStatuses.createMapping(itemId, {
