...
- 计算统计可以通过逐条处理获取到的日志记录，根据每条日志的故障码（errCode）及其对应的参数（p1、p2、p3、p4）进行不同判断：
    开机判断：检查是否满足开机条件（情况1：errCode后四位为“A01E”  情况2：errcode是否为570e 且 p1=0 且 p2≠0? 并且 s2检查下一条日志是否为571e 且 p1=0 且 p2≠0?），
        确认后标记“开机” 记录 开机时间
    关机判断：检查是否满足关机条件（情况1：errCode后四位为 ' A02E '，标记为“关机”，情况2：检查errcode=310e且p2=31，针对情况2增加条件，当检测到 errcode=310e且p2=31，记录此刻时间endTemple，在接下来的日志(时间戳-endTemple)的30s之内没有检测到errcode=310e且p1=31，p2!=31时才算真的关机）
...
1.正常开机，如果当前无手术，记录开机时间（第一次开机）
  ...
  当日志读取完成
    ...
    对于器械使用时间，连台手术时会出现，前一台手术结束，即器械状态回到0或-1；下一场手术未开始时器械状态已经变化，安装了器械，此段器械使用时间没有正确记录在下一场手术中
...
- 解析后每一个数据列具有物理意义，可以通过图表展示；
    - 前端UI显示主控制控制臂的关节位置、速度，使用eChart样式的折线图、视野下笛卡尔位置数据，使用eChart样式3D折线图；
    - 前端UI显示工具臂的关节位置、速度数据，使用eChart样式的折线图，视野下笛卡尔位置数据，使用eChart样式3D折线图；
    - - 每一条工具臂关节位置显示在一张图表上；左手关节位置显示在一张图表上，右手关节位置显示在一张图表上
    - 每一条工具臂关节速度显示在一张图表上；左手关节速度显示在一张图表上，右手关节速度显示在一张图表上
    - 每一条工具臂关笛卡尔位置显示在一张图表上；左手关节笛卡尔位置显示在一张图表上，右手笛卡尔位置显示在一张图表上
    - 支持数据回放功能，根据左手的笛卡尔位置，使用连杆，仿真机械臂的运动轨迹，可以点击回复过程；
    - 机械臂仿真的DH模型可以通过外部文件配置，需要有主控制的机械臂dh模型参数，工具臂dh模型参数、调整臂dh模型参数
