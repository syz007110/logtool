...
- 日志分析辅助功能：主要是为了工程师可以方便分析日志，针对日志列表中的日志条目进行操作，右击日志条目出现工具栏，工具栏的操作包括
    - 标记颜色，支持红、黄、蓝、绿四种颜色，零时标记页面刷新后清空；
    - 标记备注，专家和管理员用户拥有标记权限，普通用户只能查看，备注需要写入数据库？，任何用户索引到该日志都可以显示所有的备注需要区分备注的软件人，备注限制50个字，每个用户可以删除自己的标记，管理员可以删除所有标记，每一条日志支持最多5条备注；
    - 添加日志到日志分析剪切板，添加后，左侧抽屉侧边栏显示，日志条目的内容被复制到侧边栏内的剪贴板中，侧边栏仅显示剪贴板的缩略图，点击剪贴板后以弹窗形式展开方放大，剪贴板的内容可以编辑，支持另存为txt文件，缩略图中的剪贴板有删除按钮缩略图的右上角；
    - 上下文分析，点击上下文分析功能，显示数据输入框需要用户输入前xx分钟，后Xx分钟；确认后以该日志条目为基点，选择前XX分钟后XX分析的日志；清空筛选器后复位；
    - 统计此条日志在所选日志中出现的次数；右击选择统计数量，日志中故障码右上方显示上该故障码在批量日志查看中出现的次数；
- 以此条日志的参数a为y轴，时间戳为x轴，进行可视化；点击可视化后会在侧边栏仅显示剪贴板的缩略图，点击后放大到视野中间支持另存为图片格式；
- 支持隐藏某行日志，
