...
移动端适配使用 轻量 PWA + Vue 移动端 UI（Vant）的方案，共用后端

| 层 | 技术 | 说明 |
| --- | --- | --- |
| **UI 层** | Vant + Vue 3 | 移动端组件库（卡片式操作、Tab导航） |
| 体验层 | 轻量 PWA（manifest + 全屏 + 图标） | App-like 体验 |
| 通信层 | 通信层 | 通信层 |
| 部署层 | 部署层 | 部署层 |
| 国际化 / 权限 | 国际化 / 权限 | 国际化 / 权限 |

前端页面包括：

- 登录页
- 底部导航：故障码查询，日志管理，手术数据，个人中心
    - 故障码查询：用户输入故障码类型或故障码，查询方式和故障码管理页面的故障码查询一致；
    - 日志管理：以设备编号分类，前端显示：设备编号|医院名称|日志数量
        - 点击设备编号后进入该设备的日志列表页面；前端显示：日志文件名|上传用户ID|状态|操作
            - 操作列：查看|重新解析|删除
    - 手术数据：以设备编号分类，前端显示：设备编号|医院名称|手术数量
    - 点击设备编号后进入该设备的手术数据列表页面；前端显示：手术id|手术开始时间|手术结束时间|操作
        - 操作列：可视化|删除

需要支持中文和英文的多语言i18n，不修改后端；