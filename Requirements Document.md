...
- 手术分析分析日志时，应该针对的是日志分析页面中的所有日志，统计出一场手术开始：新建一个标签页，一场手术从开始到结束的数据显示在一个标签页中，
- 手术分析页面应该显示手术时长的时间轴在器械使用时间的上方，每个器械的使用时长与手术时长的时间轴需要对齐；器械使用时长应该在手术时长同一个框内，但最为子项
- 每一个标签页代表一场手术；手术序号依次累加
- 计算统计可以通过逐条处理获取到的日志记录，根据每条日志的故障码（errCode）及其对应的参数（p1、p2、p3、p4）进行不同分支判断：
- 支持显示开机时间，关机时间，手术开始时间，手术结束时间
    - 开机时间：
    - 关机时间：
