...
- 日志条目表格，表头：|标记|时间戳(文件名)|故障码|释义|参数(1~4)|操作|；
    ...
    - 操作：上下文分析，日志抓取，备注
    - 日志摘取：**日志摘取**是一个 **单一容器**，最多存 50 条日志，缩略图显示的是「剪贴板」整体，而不是一条条日志，缩略图会自动显示在右侧抽屉日志摘取标签页中；添加日志到日志分析剪切板后，侧右抽屉侧边栏显示，日志条目的内容格式为，”时间戳 故障码 释义“的文本被复制到侧边栏内的日志摘取中，侧边栏仅显示日志摘取的缩略图，点击日志摘取后以弹窗形式展开放大，日志摘取的内容可以编辑，支持另存为txt文件，鼠标悬浮至日志摘取的缩略图时右上角显示删除按钮（使用垃圾桶icon）形式与图表缩略图删除按钮一致；
...
- 计算统计可以通过逐条处理获取到的日志记录，根据每条日志的故障码（errCode）及其对应的参数（p1、p2、p3、p4）进行不同分支判断，具体处理过程根据[日志读取过程流程图](https://www.notion.so/259cc02ebc29801e8c8ccc4e749bbd2c?pvs=21)
    ...
    - 测试页面设有可视化按钮-将surgeries数据转变为可视化视图，只使用对应的surgeries数据
    ...
    - 器械信息与本场手术的起止时间，绘制一个类似甘特图的图表，x轴是本场手术的时间轴，手术时间线位于图表下方，与时间轴对齐；
        ...
        - 鼠标悬停 → 提示 **UDI码 / 使用时长**
    - **核心视图：时间轴 + 甘特图组合**
        - **时间线（横向）**：显示关键事件（开机、关机、手术开始、手术结束）以时间线形式；避免出现横向滚动条
        - 时间轴（横向）：位于时间线下侧，在时间上对齐；
            - **X轴 = 时间**（范围：手术开始前的开机时间 ~ 手术结束后的关
