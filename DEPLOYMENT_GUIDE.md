...
本项目使用语义化版本（Semantic Versioning）进行版本管理，格式为：`主版本号.次版本号.修订号`（例如：`1.2.3`）

## 📁 版本文件位置

项目中的版本号存储在以下位置：

1. **`VERSION`** - 根目录版本文件（单一真实来源）
2. **`package.json`** - 根目录 package.json 中的 version 字段
...
```bash
# 1. 更新 VERSION 文件
# 编辑 VERSION 文件，修改版本号（例如：0.1.2）

# 2. 同步更新根目录 package.json
# 编辑 package.json，更新 "version" 字段

# 3. 更新 CHANGELOG.md
# 在 CHANGELOG.md 顶部添加新版本条目，记录变更内容
```
